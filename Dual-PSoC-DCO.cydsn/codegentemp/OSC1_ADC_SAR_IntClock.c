/*******************************************************************************
* File Name: OSC1_ADC_SAR_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_Start
********************************************************************************
*
* Summary:
...
void OSC1_ADC_SAR_IntClock_Start(void) 
{
    ...
    OSC1_ADC_SAR_IntClock_CLKEN |= OSC1_ADC_SAR_IntClock_CLKEN_MASK;
	OSC1_ADC_SAR_IntClock_CLKSTBY |= OSC1_ADC_SAR_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_Stop
********************************************************************************
*
* Summary:
...
void OSC1_ADC_SAR_IntClock_Stop(void) 
{
    ...
    OSC1_ADC_SAR_IntClock_CLKEN &= (uint8)(~OSC1_ADC_SAR_IntClock_CLKEN_MASK);
	OSC1_ADC_SAR_IntClock_CLKSTBY &= (uint8)(~OSC1_ADC_SAR_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((OSC1_ADC_SAR_IntClock_CLKEN & OSC1_ADC_SAR_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = OSC1_ADC_SAR_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = OSC1_ADC_SAR_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* OSC1_ADC_SAR_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(OSC1_ADC_SAR_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(OSC1_ADC_SAR_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = OSC1_ADC_SAR_IntClock_GetSourceRegister();
    uint16 oldDivider = OSC1_ADC_SAR_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = OSC1_ADC_SAR_IntClock_CLKEN & OSC1_ADC_SAR_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(OSC1_ADC_SAR_IntClock_DIV_PTR, clkDivider);
                OSC1_ADC_SAR_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                OSC1_ADC_SAR_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(OSC1_ADC_SAR_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((OSC1_ADC_SAR_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(OSC1_ADC_SAR_IntClock_DIV_PTR, clkDivider);
				OSC1_ADC_SAR_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(OSC1_ADC_SAR_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    OSC1_ADC_SAR_IntClock_MOD_SRC |= modeBitMask & (uint8)OSC1_ADC_SAR_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    OSC1_ADC_SAR_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(OSC1_ADC_SAR_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return OSC1_ADC_SAR_IntClock_MOD_SRC & (uint8)(OSC1_ADC_SAR_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = OSC1_ADC_SAR_IntClock_GetDividerRegister();
    uint8 oldSrc = OSC1_ADC_SAR_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        OSC1_ADC_SAR_IntClock_MOD_SRC |= CYCLK_SSS;
        OSC1_ADC_SAR_IntClock_MOD_SRC =
            (OSC1_ADC_SAR_IntClock_MOD_SRC & (uint8)(~OSC1_ADC_SAR_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return OSC1_ADC_SAR_IntClock_MOD_SRC & OSC1_ADC_SAR_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    OSC1_ADC_SAR_IntClock_PHASE = clkPhase & OSC1_ADC_SAR_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: OSC1_ADC_SAR_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return OSC1_ADC_SAR_IntClock_PHASE & OSC1_ADC_SAR_IntClock_PHASE_MASK;
}
