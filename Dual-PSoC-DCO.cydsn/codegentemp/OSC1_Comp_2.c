...
uint8 OSC1_Comp_2_initVar = 0u;

/* Internal functions definitoin */
static void OSC1_Comp_2_trimAdjust(uint8 nibble) ;

/* static OSC1_Comp_2_backupStruct  OSC1_Comp_2_backup; */
#if (CY_PSOC5A)
    static OSC1_Comp_2_LOWPOWER_BACKUP_STRUCT  OSC1_Comp_2_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 OSC1_Comp_2_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: OSC1_Comp_2_Init
********************************************************************************
*
* Summary:
...
void OSC1_Comp_2_Init(void) 
{
    ...
    OSC1_Comp_2_SetSpeed(OSC1_Comp_2_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( OSC1_Comp_2_DEFAULT_HYSTERESIS == 0u )
        OSC1_Comp_2_CR |= OSC1_Comp_2_HYST_OFF;
    #else
        OSC1_Comp_2_CR &= (uint8)(~OSC1_Comp_2_HYST_OFF);
    #endif /* OSC1_Comp_2_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( OSC1_Comp_2_DEFAULT_PWRDWN_OVRD == 0u )
            OSC1_Comp_2_CR &= (uint8)(~OSC1_Comp_2_PWRDWN_OVRD);
        #else 
            OSC1_Comp_2_CR |= OSC1_Comp_2_PWRDWN_OVRD;
        #endif /* OSC1_Comp_2_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    OSC1_Comp_2_CR |= OSC1_Comp_2_MX_AO;

    /* Set default sync */
    OSC1_Comp_2_CLK &= (uint8)(~OSC1_Comp_2_SYNCCLK_MASK);
    #if ( OSC1_Comp_2_DEFAULT_BYPASS_SYNC == 0u )
        OSC1_Comp_2_CLK |= OSC1_Comp_2_SYNC_CLK_EN;
    #else
        OSC1_Comp_2_CLK |= OSC1_Comp_2_BYPASS_SYNC;
    #endif /* OSC1_Comp_2_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: OSC1_Comp_2_Enable
********************************************************************************
*
* Summary:
...
{
    OSC1_Comp_2_PWRMGR |= OSC1_Comp_2_ACT_PWR_EN;
    OSC1_Comp_2_STBY_PWRMGR |= OSC1_Comp_2_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(OSC1_Comp_2_restoreReg == 1u)
        {
            OSC1_Comp_2_CR = OSC1_Comp_2_lowPowerBackup.compCRReg;

            /* Clear the flag */
            OSC1_Comp_2_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: OSC1_Comp_2_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  OSC1_Comp_2_initVar: Is modified when this function is called for the 
...
void OSC1_Comp_2_Start(void) 
{
    ...
    if ( OSC1_Comp_2_initVar == 0u )
    {
        OSC1_Comp_2_Init();
        
        OSC1_Comp_2_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: OSC1_Comp_2_Stop
********************************************************************************
*
* Summary:
...
void OSC1_Comp_2_Stop(void) 
{
    ...
    OSC1_Comp_2_PWRMGR &= (uint8)(~OSC1_Comp_2_ACT_PWR_EN);
    OSC1_Comp_2_STBY_PWRMGR &= (uint8)(~OSC1_Comp_2_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        OSC1_Comp_2_restoreReg = 1u;

        /* Save the control register before clearing it */
        OSC1_Comp_2_lowPowerBackup.compCRReg = OSC1_Comp_2_CR;
        OSC1_Comp_2_CR = OSC1_Comp_2_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: OSC1_Comp_2_SetSpeed
********************************************************************************
*
* Summary:
...
void OSC1_Comp_2_SetSpeed(uint8 speed) 
{
    ...
    OSC1_Comp_2_CR = (OSC1_Comp_2_CR & (uint8)(~OSC1_Comp_2_PWR_MODE_MASK)) |
                           (speed & OSC1_Comp_2_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == OSC1_Comp_2_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            OSC1_Comp_2_TR = OSC1_Comp_2_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            OSC1_Comp_2_TR0 = OSC1_Comp_2_HS_TRIM_TR0;
            OSC1_Comp_2_TR1 = OSC1_Comp_2_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            OSC1_Comp_2_TR = OSC1_Comp_2_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            OSC1_Comp_2_TR0 = OSC1_Comp_2_LS_TRIM_TR0;
            OSC1_Comp_2_TR1 = OSC1_Comp_2_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: OSC1_Comp_2_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( OSC1_Comp_2_WRK & OSC1_Comp_2_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: OSC1_Comp_2_trimAdjust
********************************************************************************
*
* Summary:
...
static void OSC1_Comp_2_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = OSC1_Comp_2_WRK & OSC1_Comp_2_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                OSC1_Comp_2_TR |= OSC1_Comp_2_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                OSC1_Comp_2_TR0 |= OSC1_Comp_2_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                OSC1_Comp_2_TR |= OSC1_Comp_2_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                OSC1_Comp_2_TR1 |= OSC1_Comp_2_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((OSC1_Comp_2_WRK & OSC1_Comp_2_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: OSC1_Comp_2_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 OSC1_Comp_2_ZeroCal(void) 
{
    ...
    tmpSW0 = OSC1_Comp_2_SW0;
    tmpSW2 = OSC1_Comp_2_SW2;
    tmpSW3 = OSC1_Comp_2_SW3;

     /* Clear routing for inP, retain routing for inN */
    OSC1_Comp_2_SW0 = 0x00u;
    OSC1_Comp_2_SW2 = 0x00u;
    OSC1_Comp_2_SW3 = tmpSW3 & (uint8)(~OSC1_Comp_2_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = OSC1_Comp_2_CR;
    OSC1_Comp_2_CR |= (OSC1_Comp_2_CAL_ON | OSC1_Comp_2_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        OSC1_Comp_2_TR = OSC1_Comp_2_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        OSC1_Comp_2_TR0 = OSC1_Comp_2_DEFAULT_CMP_TRIM;
        OSC1_Comp_2_TR1 = OSC1_Comp_2_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (OSC1_Comp_2_CR & OSC1_Comp_2_PWR_MODE_MASK) == OSC1_Comp_2_PWR_MODE_FAST)
    {
        OSC1_Comp_2_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        OSC1_Comp_2_trimAdjust(1u);
		OSC1_Comp_2_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    OSC1_Comp_2_CR = tmpCR;
    
    /* Restore routing registers for inP */
    OSC1_Comp_2_SW0 = tmpSW0;
    OSC1_Comp_2_SW2 = tmpSW2;
    OSC1_Comp_2_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) OSC1_Comp_2_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)OSC1_Comp_2_TR1 << 8u) | (OSC1_Comp_2_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: OSC1_Comp_2_LoadTrim
********************************************************************************
*
* Summary:
...
void OSC1_Comp_2_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: OSC1_Comp_2_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: OSC1_Comp_2_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
